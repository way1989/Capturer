...
public final class GestureUtils {
    ...
    public static float[] spatialSampling(Gesture gesture, int bitmapSize,
        ...
        final ArrayList<android.gesture.GestureStroke> strokes = gesture.getStrokes();
        final int count = strokes.size();
        int size;
        float xpos;
        float ypos;
        for (int index = 0; index < count; index++) {
            final android.gesture.GestureStroke stroke = strokes.get(index);
    ...
    public static float[] temporalSampling(ClipStroke stroke, int numPoints) {
        final float increment = stroke.length / (numPoints - 1);
        int vectorLength = numPoints * 2;
        float[] vector = new float[vectorLength];
        float distanceSoFar = 0;
     ...
     */
//    public static OrientedBoundingBox computeOrientedBoundingBox(ArrayList<android.gesture.GesturePoint> originalPoints) {
