package com.way.captain.widget.freecrop;
...
/**
 * Created by android on 15-11-12.
 */
public class ClipStroke {
     ...
     * @param points
     */
    public ClipStroke(Rect imageRect, ArrayList<GesturePoint> points) {
        ...
        for (int i = 0; i < count; i++) {
            final GesturePoint point = points.get(i);
            GesturePoint p = new GesturePoint((point.x - (float) imageRect.left), (point.y - (float) imageRect.top), point.timestamp);
            tmpPoints[i * 2] = p.x;
            tmpPoints[i * 2 + 1] = p.y;
     ...
     */
    private ClipStroke(RectF bbx, float len, float[] pts, long[] times) {
    ...
    @Override
    public Object clone() {
        return new ClipStroke(boundingBox, length, points, timestamps);
    }
